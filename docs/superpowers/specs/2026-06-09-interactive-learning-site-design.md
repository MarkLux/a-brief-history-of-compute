...
- Ch1 人机交互与操作系统基础
- Ch2 编程语言、面向对象、Java/JVM、内存管理、线程并发
- Ch3 软件工程与分布式系统
...
- 对 JS/TS 的事件循环、原型链、闭包等概念熟悉，可以作为类比参考。
- 对后端服务端概念（进程、线程、内存模型、JVM、编译/解释、并发模型）有了解需求但不深入。
- 对 Java/JVM 世界的认知可能只有”跨平台”和”垃圾回收”两个标签。
- 对操作系统层面（文件系统、内存管理、进程调度）可能只有浏览器相关经验。

### 内容策略

- **利用前端知识做类比**：例如”事件循环 → 进程调度”、”V8 的 Mark-Sweep → JVM 的 GC”、”原型链 → 虚方法表”。
- **Ch2（编程语言）Java 部分从基础开始**：不能假设读者理解字节码、类加载机制、JVM 内存分区。**从”高级语言和低级语言的区别”讲起 → 编译过程 → JVM 作为中间层 → 再深入 Java 的内存管理和线程模型**。
- **对前端熟悉的同学，先建立”这就是你已知内容的另一面”的感知，再展现差异**。
...
- Ch2 高级语言演进：`history-timeline`，从编译/解释/运行时基础讲起
- Ch2 Java/JVM 相关内容：`concept-comparison` + `system-mechanism`，先用前端熟悉的 JS/V8/浏览器运行时作类比，再逐步引入字节码、类加载、JVM 内存模型、GC 和线程模型
